...
import {
  ArrowLeft, Gift, Users, AlertCircle, Eye, EyeOff,
...
/**
 * Offrir du crédit à un proche depuis ton solde Mixx.
 * USSD : *145*3*1*2*{phone}*{amount}*{pin}#
 */
export default function TopupOtherActionScreen({ navigation, route }) {
  ...
  const previewUSSD = useMemo(() => {
    return resolveUSSD(service.ussdTemplate, {
      pin:    pin || 'PIN',
      phone:  cleanPhone || 'NUMÉRO',
      amount: amt > 0 ? amt : 'MONTANT',
    });
  }, [service.ussdTemplate, pin, cleanPhone, amt]);

  const isValid = cleanPhone.length >= 7 && amt > 0 && pin.length >= 4;

  const handleSend = () => {
    if (!isValid) return;
    const code = resolveUSSD(service.ussdTemplate, {
      pin, phone: cleanPhone, amount: amt,
    });
    dialUSSD(code);
  };

  const handleContactPick = ({ phone: contactPhone }) => {
    setPhone(contactPhone.replace(/\D/g, ''));
  };
    ...
    >
      <View style={{ paddingTop: insets.top }}>
        <View style={s.headerBar}>
          ...
          <Text style={s.headerTitle}>Offrir du crédit</Text>
          <View style={{ width: 40 }} />
        </View>
      </View>
      ...
      >
        ...
        >
          ...
          <Text style={s.heroSub}>
            Envoie du crédit à un proche en 1 seul appel USSD depuis ton solde Mixx.
          </Text>
        </LinearGradient>
        ...
        <View style={s.form}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.88}
          >
            <Gift size={17} color="#FFF" strokeWidth={2.4} />
            <Text style={s.ctaTxt}>
              {amt > 0 ? `Offrir ${amt.toLocaleString('fr-FR')} F` : 'Offrir'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  phoneCard: {
    ...
    paddingHorizontal: 16, paddingVertical: 6, marginBottom: 16, ...SHADOW,
  },
