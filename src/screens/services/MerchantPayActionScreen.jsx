...
export default function MerchantPayActionScreen({ navigation, route }) {
  ...
  }, [service.ussdTemplate, pin, merchant, amt]);

  const isValid = merchant.length >= 3 && amt > 0 && pin.length >= 4;

  const handleSend = () => {
    if (!isValid) return;
    const code = resolveUSSD(service.ussdTemplate, { pin, merchant, amount: amt });
    dialUSSD(code);
  };
    ...
    >
      <View style={{ paddingTop: insets.top }}>
        <View style={s.headerBar}>
          ...
          <Text style={s.headerTitle}>Paiement marchand</Text>
          <View style={{ width: 40 }} />
        </View>
      </View>
      ...
      >
        ...
        >
          ...
          <Text style={s.heroOver}>{operator?.label ?? 'PAIEMENT'}</Text>
          <Text style={s.heroTitle}>{service.label}</Text>
          <Text style={s.heroSub}>
            Code marchand + montant + PIN. On compose le code USSD complet en 1 appel.
          </Text>
        </LinearGradient>

        <View style={s.form}>
          <Text style={s.fieldLabel}>CODE MARCHAND</Text>
          <View style={s.card}>
            <TextInput
              style={s.input}
              value={merchant}
              onChangeText={(t) => setMerchant(t.replace(/\D/g, ''))}
...
const s = StyleSheet.create({
  ...
  input: { fontFamily: FONT, fontSize: 16, fontWeight: '600', color: COLORS.black, padding: 0, letterSpacing: 0.5 },
